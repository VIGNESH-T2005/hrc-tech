    ...
    <div className="grid min-h-[calc(100vh-64px)] grid-cols-1 bg-[var(--bg-base)] lg:grid-cols-2">
      <div className="gradient-gold relative hidden flex-col justify-center overflow-hidden px-14 text-slate-950 lg:flex">
        ...
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
          ...
          <p className="mt-3 max-w-sm text-slate-900/70">Create a free student account and get instant access to every course you purchase.</p>
        </motion.div>
      </div>
      ...
      <div className="flex items-center justify-center px-4 py-16">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="w-full max-w-sm">
          ...
          <h2 className="text-2xl font-bold text-slate-100">Create a student account</h2>
          <p className="mt-1 text-sm text-slate-400">Takes less than a minute.</p>
          ...
          <p className="mt-6 text-center text-sm text-slate-400">
            Already have an account? <Link to="/login" className="font-semibold text-amber-400">Log in</Link>
          </p>
        </motion.div>
      </div>
    </div>
